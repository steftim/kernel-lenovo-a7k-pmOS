...
export KBUILD_BUILD_HOST="HYPERLAPSE"
DEVICE="aio_row"
...
dd_zip() {
                cd $AK
                BUILD_TIME=$(date +"%H%M")
                zip -r Venom-MAX-OC-$VAR-$BUILD_DATE-$BUILD_TIME-$DEVICE.zip *
                if ! [ -d "$OUTPUTDIR" ]; then
                mkdir $OUTPUTDIR
                fi;
                mv Venom-MAX-OC.zip $OUTPUTDIR
                cd $TREE
                }
...
echo "$green(2) Compile VENOM-MAX-OC build"
