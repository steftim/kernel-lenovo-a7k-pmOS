...
static bool dt2w_phone_call_override = false;
...
static ssize_t show_dt2w_phone_call_override(struct kobject *kobj, struct attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", dt2w_phone_call_override);
}

static ssize_t store_dt2w_phone_call_override(struct kobject *kobj, struct attribute *attr, const char *buf, size_t count)
{
	ssize_t res;
	unsigned long input;
	res = strict_strtoul(buf, 0, &input);
	if (res >= 0 && input >= 0 && input <= 1) {
		dt2w_phone_call_override = input;
        phone_call_override = dt2w_phone_call_override;
		if (!suspended)
			thunderx_update_min_max_allcpus();
	}
	else return -EINVAL;
...
define_global_rw_attr(dt2w_phone_call_override);
...
static struct attribute * thunderx_attributes[] = {
    ...
    &dt2w_phone_call_override_attr.attr,
	NULL,
};
...
// ********* Venom kernel additions for A7000 (aio_row) inits **********
    ...
    dt2w_phone_call_override = false;
