...
static unsigned int vib, x_left, x_right, y_up, y_down, gesture_min_x, gesture_min_y, gesture_sensitivity, gesture_vib;
...
static ssize_t show_gesture_sensitivity(struct kobject *kobj, struct attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", gesture_sensitivity);
}

static ssize_t store_gesture_sensitivity(struct kobject *kobj, struct attribute *attr, const char *buf, size_t count)
{
	ssize_t res;
	unsigned long input;
	res = strict_strtoul(buf, 0, &input);
	if (res >= 0 && input > 0 && input <= 10) {
		gesture_sensitivity = input;
        min_delta = gesture_sensitivity;
		if (!suspended)
			thunderx_update_min_max_allcpus();
	}
	else return -EINVAL;
...
define_global_rw_attr(gesture_sensitivity);
...
// ********* Venom kernel additions for A7000 (aio_row) inits **********
    ...
    pocket_mode_dt2w_override_taps_timeout = 450;
