...
static unsigned int mt_gpufreq_get_dvfs_table_type(void)
{
	...
	return 0; //Returning type 0 (700 region) on OC
#endif
...
#ifdef CONFIG_MTK_GPU_OC
		if(!allow_gpu_oc)
			g_pGpufreq_input_boost_notify(2);
		else
#endif		
...
unsigned int mt_gpufreq_target(unsigned int idx)
{
     ...
     *************************************************/
#ifdef CONFIG_MTK_GPU_OC
	if (!allow_gpu_oc)		//Handle Overclock
		if(target_freq > mt_gpufreqs[2].gpufreq_khz)
		{
			target_freq = mt_gpufreqs[2].gpufreq_khz;
			target_volt = mt_gpufreqs[2].gpufreq_volt;
		}
#endif		
...
#ifdef MT_GPUFREQ_LOW_BATT_VOLUME_PROTECT
#undef MT_GPUFREQ_LOW_BATT_VOLUME_PROTECT
#endif
...
#ifdef MT_GPUFREQ_LOW_BATT_VOLT_PROTECT
#undef MT_GPUFREQ_LOW_BATT_VOLT_PROTECT
#endif
...
#ifdef CONFIG_MTK_GPU_OC
static int mt_allow_gpu_oc_700_proc_show(struct seq_file *m, void *v)
{
    if (allow_gpu_oc)
        seq_printf(m, "1\n");
    else
...
static ssize_t mt_allow_gpu_oc_700_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    char desc[3];
    int len = 0;
    ...
    if (sscanf(desc, "%d", &allow_oc) == 1) {
        if ((allow_oc == 0) || (allow_oc == 1))
            allow_gpu_oc = allow_oc;
        else
            gpufreq_warn("bad argument!! should be 0 or 1 [0: disable, 1: enable]\n");
    }
    else
