...
static int _mt_cpufreq_pdrv_probed = 0;
...
static unsigned int _mt_cpufreq_get_cpu_level(void)
{
    ...
    return CPU_LEVEL_2;	// force LEVEL_2
#else
    {
...
static struct pmic_wrap_setting pw = {
    ...
    .set[PMIC_WRAP_PHASE_SUSPEND] = {
        ...
        ._[IDX_SP_VCORE_TEMP1]      = { PMIC_ADDR_VCORE_VOSEL_ON,   VOLT_TO_PMIC_VAL(96250),  }, /* voltage needs to sync with mt_spm_sleep.c */
        ._[IDX_SP_VCORE_TEMP2]      = { PMIC_ADDR_VCORE_VOSEL_ON,   VOLT_TO_PMIC_VAL(93125),  }, /* voltage needs to sync with mt_spm_sleep.c */
        .nr_idx = NR_IDX_SP,
    },
...
void mt_cpufreq_set_pmic_cmd(enum pmic_wrap_phase_id phase, int idx, unsigned int cmd_wdata) /* just set wdata value */
{
	...
	if (GPU800MHz_DRAM1866MHz_Flag) {
		if ((phase == PMIC_WRAP_PHASE_NORMAL) && (idx == IDX_NM_VCORE)) {
			if (PMIC_VAL_TO_VOLT(cmd_wdata) > 105000) {
				cmd_wdata = VOLT_TO_PMIC_VAL(105000);
			}
		}
	}
	/* Turbo Dram/GPU E */
...
void mt_cpufreq_set_pmic_cmd_full(enum pmic_wrap_phase_id phase, int idx, unsigned int cmd_addr, unsigned int cmd_wdata) /* just set wdata value */
{
    unsigned long flags;
    ...
    //cpufreq_dbg("@%s: phase = 0x%x, idx = %d, cmd_addr = 0x%x, cmd_wdata = 0x%x\n", __func__, phase, idx, cmd_addr, cmd_wdata);
    ...
    if (pw.phase == phase) {
        cpufreq_write(pw.addr[idx].cmd_addr, cmd_addr);
        cpufreq_write(pw.addr[idx].cmd_wdata, cmd_wdata);
    }
...
EXPORT_SYMBOL(mt_cpufreq_set_pmic_cmd_full);
...
#ifdef CONFIG_MTK_CPU_OC
/* CPU LEVEL 2, 1.7GHz segment */
static struct mt_cpu_freq_info opp_tbl_e1_2[] = {
    OP(CPU_DVFS_FREQ2,  92000),
    OP(CPU_DVFS_FREQ4,  92000),
    OP(CPU_DVFS_FREQ4_2, 91500),
    OP(CPU_DVFS_FREQ5,   90625),
    OP(CPU_DVFS_FREQ6,   88125),
    OP(CPU_DVFS_FREQ7,   86250),
    OP(CPU_DVFS_FREQ8,   82500),
    OP(CPU_DVFS_FREQ10,  80000),
};
#else
...
static void _set_no_limited(struct mt_cpu_dvfs *p)
{
	...
	p->limited_max_freq_by_user = 1495000;
            ...
            for (ncpu = possible_cpu; ncpu > 0; ncpu--) {
                for (i = 0; i < p->nr_opp_tbl * possible_cpu; i++) {
                    if (p->power_tbl[i].cpufreq_power <= limited_power) { // p->power_tbl[i].cpufreq_ncpu == ncpu &&
                        ...
                        if (!init)                       
                            p->limited_max_freq = 1495000;      //p->power_tbl[i].cpufreq_khz;
                        else
...
{
	if (boost && _mt_cpufreq_pdrv_probed)
...
static int mtk_cpufreq_throttle = 0;
static int mtk_cpunum_throttle = 1;
static int mtk_cpunum_throttle_num = 4;
static int mtk_cpunum_throttle_battery_limit = 15;
...
{       			
    ...
    //  
    // Add throttling customisability
    //
    
    kal_uint32 batt_percentage_present = bat_get_ui_percentage();
    
    if((mtk_cpufreq_throttle) && (batt_percentage_present <= mtk_cpunum_throttle_battery_limit))
    ...
    // Add throttling customisability
    //
    if((mtk_cpunum_throttle) && (batt_percentage_present <= mtk_cpunum_throttle_battery_limit))
         // Limit CPU core num to cpunum
         hps_set_cpu_num_limit(LIMIT_LOW_BATTERY, mtk_cpunum_throttle_num, 0);
    else if (!(p->pwr_thro_mode & PWR_THRO_MODE_LBAT_936MHZ))                 //Unlimit CPU core num if no Lbat_protect_limit
...
void check_pmic_6325_version(void)
{
    ...
    if (first_check == 1)
    {
		...
		cpufreq_info("gPMIC_6325_version = 0x%x\n", gPMIC_6325_version);
    }
}
...
#define DORMANT_MODE_VOLT   80000
        ...
        policy->max = 1495000; //cpu_dvfs_get_max_freq(id_to_cpu_dvfs(id));
...
/* find how many actual entries there are */
	...
	out += sprintf(out, "%umhz: %d mV\n",
			opp_tbl_e1_2[i].cpufreq_khz/1000,	// in MHz
			opp_tbl_e1_2[i].cpufreq_volt/100);	// in mV
    }
...
ssize_t store_UV_mV_table(struct cpufreq_policy *policy, const char *buf, size_t count)
{
	...
	opp_tbl_e1_2[i].cpufreq_volt = u[i]*100;	// to mV *100
	//opp_tbl_e1_2[i].cpufreq_volt_org = u[i]*100;	// seems not need this
    }    

    _mt_cpufreq_setup_freqs_table(policy, &opp_tbl_e1_2, NR_MAX_OPP_TBL);
    return count;
}
...
static ssize_t cpufreq_limited_by_hevc_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
    ...
    if (sscanf(buf, "%d", &limited_freq_by_hevc) == 1) {
        ...
        if (cpu_dvfs_is_availiable(p) && (p->limited_freq_by_hevc > cpu_dvfs_get_cur_freq(p))) {
            ...
            if (policy) {
				cpufreq_dbg("@%s: limited_freq_by_hevc = %d\n",
						__func__, p->limited_freq_by_hevc);
...
void set_limit_max_freq_by_user(unsigned int limited_max_freq)
{
    ...
    {            
        ...
        else if (limited_max_freq > 1690000)
            limited_max_freq = 1690000;   
    }               
    else
        return;                     
...
/*    struct mt_cpu_dvfs *p = (struct mt_cpu_dvfs *)PDE_DATA(file_inode(file));
    ...
    if (sscanf(buf, "%d", &limited_max_freq) == 1) {
        ...
        if (cpu_dvfs_is_availiable(p) && (p->limited_max_freq_by_user != 0) && (p->limited_max_freq_by_user < cpu_dvfs_get_cur_freq(p))) {
            ...
            if (policy) {
				cpufreq_dbg("@%s: limited_max_freq_by_user = %d\n",
						__func__, p->limited_max_freq_by_user);
                cpufreq_driver_target(policy, p->limited_max_freq_by_user, CPUFREQ_RELATION_H);
                cpufreq_cpu_put(policy);
            }
        }
 ...
 /*       if (cpu_dvfs_is_availiable(p) && (p->limited_max_freq != 0) && (p->limited_max_freq < cpu_dvfs_get_cur_freq(p))) {
            struct cpufreq_policy *policy = cpufreq_cpu_get(p->cpu_id);

            if (policy) {
				cpufreq_dbg("@%s: limited_max_freq_by_user = %d\n",
						__func__, p->limited_max_freq);
                cpufreq_driver_target(policy, p->limited_max_freq, CPUFREQ_RELATION_H);
                cpufreq_cpu_put(policy);
            }
        }
    } else
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Custom cpufreq tuning
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

static int cpufreq_mtk_cpufreq_throttle_proc_show(struct seq_file *m, void *v)
{
    seq_printf(m, "%d\n", mtk_cpufreq_throttle);
    return 0;
}

static ssize_t cpufreq_mtk_cpufreq_throttle_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
    int throttle = 1;
    char *buf = _copy_from_user_for_proc(buffer, count);
    ...
    if (sscanf(buf, "%d", &throttle) == 1) {
    	if ((throttle == 0) || (throttle == 1))
			mtk_cpufreq_throttle = throttle;
    }
    
    bat_per_protection_powerlimit(BATTERY_PERCENT_LEVEL_1);

    free_page((unsigned long)buf);
    return count;
}

static int cpufreq_mtk_cpunum_throttle_proc_show(struct seq_file *m, void *v)
{
    seq_printf(m, "%d\n", mtk_cpunum_throttle);
    return 0;
}

static ssize_t cpufreq_mtk_cpunum_throttle_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
    int throttle = 1;
    char *buf = _copy_from_user_for_proc(buffer, count);
...
static int cpufreq_mtk_cpunum_throttle_num_proc_show(struct seq_file *m, void *v)
{
    seq_printf(m, "%d\n", mtk_cpunum_throttle_num);
    return 0;
}

static ssize_t cpufreq_mtk_cpunum_throttle_num_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
    int throttle_num = 4;
    char *buf = _copy_from_user_for_proc(buffer, count);
    ...
    if (sscanf(buf, "%d", &throttle_num) == 1) {
        if ((throttle_num > 0) && (throttle_num < max_cpu_num))
			mtk_cpunum_throttle_num = throttle_num;
    }
...
static int cpufreq_mtk_cpunum_throttle_battery_limit_proc_show(struct seq_file *m, void *v)
{
    seq_printf(m, "%d\n", mtk_cpunum_throttle_battery_limit);
    return 0;
}

static ssize_t cpufreq_mtk_cpunum_throttle_battery_limit_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
    int throttle_batt = 4;
    char *buf = _copy_from_user_for_proc(buffer, count);
    ...
    if (sscanf(buf, "%d", &throttle_batt) == 1) {
        if ((throttle_batt > 0) && (throttle_batt < 100))
			mtk_cpunum_throttle_battery_limit = throttle_batt;
    }
...
PROC_FOPS_RW(cpufreq_mtk_cpufreq_throttle); // <-custom
PROC_FOPS_RW(cpufreq_mtk_cpunum_throttle); // <-custom
PROC_FOPS_RW(cpufreq_mtk_cpunum_throttle_num); // <-custom
PROC_FOPS_RW(cpufreq_mtk_cpunum_throttle_battery_limit); // <-custom
...
static int _create_procfs(void)
{
    ...
    const struct pentry cpu_entries[] = {
        ...
        PROC_ENTRY(cpufreq_mtk_cpufreq_throttle), // <-custom
        PROC_ENTRY(cpufreq_mtk_cpunum_throttle), // <-custom
        PROC_ENTRY(cpufreq_mtk_cpunum_throttle_num), // <-custom
        PROC_ENTRY(cpufreq_mtk_cpunum_throttle_battery_limit), // <-custom
    };
