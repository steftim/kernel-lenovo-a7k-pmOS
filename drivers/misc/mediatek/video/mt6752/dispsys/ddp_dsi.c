...
{	\
	...
	while \
	(INREG32(MIPITX_BASE+0xC)&0x1 != 0x1);		\
...
#define DSI_POLLREG32(cmdq, addr, mask, value)  cmdqRecPoll(cmdq, ((unsigned long)addr)&0x1fffffff, value, mask);
...
static void _DSI_INTERNAL_IRQ_Handler(DISP_MODULE_ENUM module,
				      unsigned int param)
{
	int i = 0;
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (status.TE_RDY) {
			DSI_OUTREG32(NULL, &txrx_ctrl,
				     INREG32(&DSI_REG[i]->DSI_TXRX_CTRL));
		...
		if (status.SLEEPOUT_DONE) {
			...
			wake_up_interruptible(&_dsi_wait_sleep_out_done_queue
					      [i]);
		}
		if (status.FRAME_DONE_INT_EN) {
			/* MMProfileLogEx(ddp_mmp_get_events()->dsi_frame_done, MMProfileFlagPulse,
			   DSI_REG[0]->DSI_LFR_STA.LFR_SKIP_CNT, DSI_REG[0]->DSI_LFR_STA.LFR_SKIP_STA); */
		}
	}
}
...
static DSI_STATUS DSI_Reset(DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		DSI_OUTREGBIT(cmdq, DSI_COM_CTRL_REG, DSI_REG[i]->DSI_COM_CTRL,
			      DSI_RESET, 1);
		DSI_OUTREGBIT(cmdq, DSI_COM_CTRL_REG, DSI_REG[i]->DSI_COM_CTRL,
			      DSI_RESET, 0);
	}
...
static DSI_STATUS DSI_SetMode(DISP_MODULE_ENUM module, cmdqRecHandle cmdq,
			      unsigned int mode)
{
	int i = 0;

	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		DSI_OUTREGBIT(cmdq, DSI_MODE_CTRL_REG,
...
static void DSI_WaitForNotBusy(DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
	...
	if (cmdq) {
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
		     i++) {
			DSI_POLLREG32(cmdq, &DSI_REG[i]->DSI_INTSTA, 0x80000000,
				      0x0);
		}
		return;
	}
	...
	while (1) {
		...
		if ((end_time - start_time) / 1000 > 2000000) {	/* 1s timeout */
			DISPERR
			    ("dsi wait not busy timeout,start at %lld, end at %lld\n",
			     start_time, end_time);
...
static DSI_STATUS DSI_SetVdoFrmMode(DISP_MODULE_ENUM module,
					    cmdqRecHandle cmdq,
					    unsigned int mode)
{
		int i = 0;
		...
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
		     i++) {
			DSI_OUTREGBIT(cmdq, DSI_MODE_CTRL_REG,
				      DSI_REG[i]->DSI_MODE_CTRL, FRM_MODE,
				      mode);
		}
...
static DSI_STATUS DSI_SetSwitchMode(DISP_MODULE_ENUM module,
					    cmdqRecHandle cmdq,
					    unsigned int mode)
{
		int i = 0;
...
static DSI_STATUS DSI_SetBypassRack(DISP_MODULE_ENUM module,
					    cmdqRecHandle cmdq,
					    unsigned int bypass)
{
		int i = 0;
		...
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
		     i++) {
			if (bypass == 0) {
				DSI_OUTREGBIT(cmdq, DSI_RACK_REG,
					      DSI_REG[i]->DSI_RACK,
					      DSI_RACK_BYPASS, 0);
			} else
				DSI_OUTREGBIT(cmdq, DSI_RACK_REG,
...
DSI_STATUS DSI_DisableClk(DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
		...
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
		     i++) {
			DSI_OUTREGBIT(cmdq, DSI_COM_CTRL_REG,
				      DSI_REG[i]->DSI_COM_CTRL, DSI_EN, 0);
		}
#endif
		return DSI_STATUS_OK;
	}

void DSI_lane0_ULP_mode(DISP_MODULE_ENUM module, cmdqRecHandle cmdq,
				bool enter)
{
		int i = 0;
		...
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
		     i++) {
			if (enter) {
				DSI_OUTREGBIT(cmdq, DSI_PHY_LD0CON_REG,
					      DSI_REG[i]->DSI_PHY_LD0CON,
					      L0_HS_TX_EN, 0);
				mdelay(1);
				DSI_OUTREGBIT(cmdq, DSI_PHY_LD0CON_REG,
					      DSI_REG[i]->DSI_PHY_LD0CON,
...
void DSI_clk_ULP_mode(DISP_MODULE_ENUM module, cmdqRecHandle cmdq,
			      bool enter)
{
		int i = 0;
		...
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
		     i++) {
			if (enter) {
				DSI_OUTREGBIT(cmdq, DSI_PHY_LCCON_REG,
...
bool DSI_clk_HS_state(DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
		...
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
		     i++) {
			DSI_READREG32(PDSI_PHY_LCCON_REG, &tmpreg,
				      &DSI_REG[i]->DSI_PHY_LCCON);
			return tmpreg.LC_HS_TX_EN ? TRUE : FALSE;
		}
}

void DSI_clk_HS_mode(DISP_MODULE_ENUM module, cmdqRecHandle cmdq,
			     bool enter)
{
		int i = 0;
...
DSI_STATUS DSI_DumpRegisters(DISP_MODULE_ENUM module, int level)
{
		...
		if (level >= 0) {
			if (module == DISP_MODULE_DSI0
				...
				DDPDUMP("DSI0 state:%s\n",
					...
					(DSI_DBG6_Status));
				DDPDUMP
				    ("DSI Mode: lane num: transfer count: status: ");
				DDPDUMP
				    ("DSI0 LFR En:%d, LFR MODE:%d, LFR TYPE:%d, LFR SKIP NUMBER:%d\n",
		...
		if (level >= 1) {
			...
			if (module == DISP_MODULE_DSI1
				...
				DDPDUMP
				    ("---------- Start dump DSI1 registers ----------\n");

				for (i = 0; i < sizeof(DSI_REGS); i += 16) {
					DDPDUMP
					    ("DSI+%04x : 0x%08x  0x%08x  0x%08x  0x%08x\n",
					     i, INREG32(DSI1_BASE + i),
...
int DSI_WaitVMDone(DISP_MODULE_ENUM module)
{
		...
		if (_dsi_is_video_mode(module)) {
			pr_debug
			    ("DSI_WaitVMDone error: should set DSI to CMD mode firstly\n");
			return;
		}
...
static void DSI_WaitForNotBusy(DISP_MODULE_ENUM module,
				       cmdqRecHandle cmdq)
{
		int i = 0;
		unsigned int count = 0;
...
DSI_STATUS DSI_SleepOut(DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
		...
		int wake_up_prd =
		    (_dsi_context[i].dsi_params.PLL_CLOCK * 2 * 1000) / (1024 *
									 8) +
		    0x1;
		...
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
			...
			DSI_OUTREGBIT(cmdq, DSI_TIME_CON0_REG,
...
DSI_STATUS DSI_Wakeup(DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
		...
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
			...
			DSI_OUTREGBIT(cmdq, DSI_START_REG,
			...
			do {
				...
				ret =
				    wait_event_interruptible_timeout
				    (_dsi_wait_sleep_out_done_queue[i],
			...
			if (ret == 0) {
				...
				DISPERR
				    ("dsi wait sleep out weake up by signal ret %d\n",
				     ret);
				mdelay(5);
			}
			DSI_OUTREGBIT(cmdq, DSI_START_REG,
...
DSI_STATUS DSI_BackupRegisters(DISP_MODULE_ENUM module,
				       cmdqRecHandle cmdq)
{
		int i = 0;
		DSI_REGS *regs = NULL;
		...
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
			...
			DSI_OUTREG32(cmdq, &regs->DSI_PSCTRL,
				     AS_UINT32(&DSI_REG[i]->DSI_PSCTRL));

			DSI_OUTREG32(cmdq, &regs->DSI_VSA_NL,
				     AS_UINT32(&DSI_REG[i]->DSI_VSA_NL));
			DSI_OUTREG32(cmdq, &regs->DSI_VBP_NL,
				     AS_UINT32(&DSI_REG[i]->DSI_VBP_NL));
			DSI_OUTREG32(cmdq, &regs->DSI_VFP_NL,
				     AS_UINT32(&DSI_REG[i]->DSI_VFP_NL));
			DSI_OUTREG32(cmdq, &regs->DSI_VACT_NL,
				     AS_UINT32(&DSI_REG[i]->DSI_VACT_NL));

			DSI_OUTREG32(cmdq, &regs->DSI_HSA_WC,
				     AS_UINT32(&DSI_REG[i]->DSI_HSA_WC));
			DSI_OUTREG32(cmdq, &regs->DSI_HBP_WC,
				     AS_UINT32(&DSI_REG[i]->DSI_HBP_WC));
			DSI_OUTREG32(cmdq, &regs->DSI_HFP_WC,
				     AS_UINT32(&DSI_REG[i]->DSI_HFP_WC));
			DSI_OUTREG32(cmdq, &regs->DSI_BLLP_WC,
				     AS_UINT32(&DSI_REG[i]->DSI_BLLP_WC));
			...
			pr_debug
			    ("DSI_BackupRegisters VM_CMD_EN %d TS_VFP_EN %d\n",
...
DSI_STATUS DSI_RestoreRegisters(DISP_MODULE_ENUM module,
					cmdqRecHandle cmdq)
{
		int i = 0;
		DSI_REGS *regs = NULL;
		...
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
			...
			DSI_OUTREG32(cmdq, &DSI_REG[i]->DSI_INTEN,
				     AS_UINT32(&regs->DSI_INTEN));
			...
			pr_debug
			    ("DSI_RestoreRegisters VM_CMD_EN %d TS_VFP_EN %d\n",
...
DSI_STATUS DSI_BIST_Pattern_Test(DISP_MODULE_ENUM module,
					 cmdqRecHandle cmdq, bool enable,
					 unsigned int color)
{
		int i = 0;
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
			...
			if (enable) {
				...
				if (!_dsi_is_video_mode(module)) {
					...
					DSI_OUTREG32(cmdq,
						     &DSI_CMDQ_REG[i]->data[0],
						     AS_UINT32(&t0));
					DSI_OUTREG32(cmdq,
						     &DSI_REG[i]->DSI_CMDQ_SIZE,
						     1);

					/* DSI_OUTREGBIT(DSI_START_REG,DSI_REG->DSI_START,DSI_START,0); */
					DSI_OUTREG32(cmdq,
						     &DSI_REG[i]->DSI_START, 0);
					DSI_OUTREG32(cmdq,
						     &DSI_REG[i]->DSI_START, 1);
				...
				DSI_OUTREG32(cmdq, &DSI_REG[i]->DSI_BIST_CON,
					     0x00);
			}
				   ...
				   LCM_DSI_PARAMS *dsi_params)
{
		...
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
			...
			ASSERT(dsi_params->horizontal_frontporch *
			       dsiTmpBufBpp > 11);
			horizontal_frontporch_byte =
			    (dsi_params->horizontal_frontporch * dsiTmpBufBpp -
			     12);
...
void DSI_Set_LFR(DISP_MODULE_ENUM module, cmdqRecHandle cmdq,
			 ...
			 unsigned int enable, unsigned int skip_num)
{
		...
		unsigned int i = 0;
		pr_debug("module=%d,mode=%d,type=%d,enable=%d,skip_num=%d\n",
			module, mode, type, enable, skip_num);
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
		     i++) {
			DSI_OUTREGBIT(cmdq, DSI_LFR_CON_REG,
...
int DSI_LFR_Status_Check(void)
{
		...
		DISPCHECK("LFR_SKIP_CNT 0x%x LFR_SKIP_STA 0x%x,status 0x%x\n",
			  lfr_skip_sta.LFR_SKIP_CNT, lfr_skip_sta.LFR_SKIP_STA,
			  status);
				  ...
				  LCM_DSI_PARAMS *dsi_params, int w, int h)
{
		...
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
		     i++) {
			DSI_OUTREGBIT(cmdq, DSI_VACT_NL_REG,
			...
			if (dsi_params->ufoe_enable
				...
				if (dsi_params->ufoe_params.compress_ratio == 3) {	/* 1/3 */
					unsigned int ufoe_internal_width =
					    w + w % 4;
					...
					DSI_OUTREGBIT(cmdq, DSI_PSCTRL_REG,
				    ...
				    LCM_DSI_PARAMS *dsi_params)
{
		...
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
		     i++) {
			DSI_OUTREGBIT(cmdq, DSI_TXRX_CTRL_REG,
...
int MIPITX_IsEnabled(DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
		...
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
		     i++) {
			if (DSI_PHY_REG[i]->
			    MIPITX_DSI_PLL_CON0.RG_DSI0_MPPLL_PLL_EN)
				ret++;
		}
...
unsigned int dsi_phy_get_clk(DISP_MODULE_ENUM module)
{
		int i = 0;
		unsigned int pcw =
		    DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON2.RG_DSI0_MPPLL_SDM_PCW_H;
		unsigned int prediv =
		    (1 <<
		     (DSI_PHY_REG[i]->
		      MIPITX_DSI_PLL_CON0.RG_DSI0_MPPLL_PREDIV));
		unsigned int posdiv =
		    (1 <<
		     (DSI_PHY_REG[i]->
		      MIPITX_DSI_PLL_CON0.RG_DSI0_MPPLL_POSDIV));
		unsigned int txdiv0 =
		    (1 <<
		     (DSI_PHY_REG[i]->
		...
		DISPMSG
		    ("%s, pcw: %d, prediv: %d, posdiv: %d, txdiv0: %d, txdiv1: %d\n",
				 ...
				 LCM_DSI_PARAMS *dsi_params)
{
		...
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
			...
			DISPMSG
			    ("PLL config: Int=0x%x, LNT=0x%x,clk=0x%x,lan3=0x%x,lan2=0x%x,lan1=0x%x,lan0=0x%x\n",
			     lnt,
			...
			/* step 2*/
			MIPITX_OUTREGBIT(MIPITX_DSI_BG_CON_REG,
					 DSI_PHY_REG[i]->MIPITX_DSI_BG_CON,
					 RG_DSI_BG_CORE_EN, 1);
			MIPITX_OUTREGBIT(MIPITX_DSI_BG_CON_REG,
					 DSI_PHY_REG[i]->MIPITX_DSI_BG_CON,
					 RG_DSI_BG_CKEN, 1);
			...
			/* step 4*/
			MIPITX_OUTREGBIT(MIPITX_DSI_TOP_CON_REG,
					 DSI_PHY_REG[i]->MIPITX_DSI_TOP_CON,
					 RG_DSI_LNT_HS_BIAS_EN, 1);

			/* step 5*/
			MIPITX_OUTREGBIT(MIPITX_DSI_CON_REG,
					 DSI_PHY_REG[i]->MIPITX_DSI_CON,
			...
			/* step 6*/
			MIPITX_OUTREGBIT(MIPITX_DSI_PLL_PWR_REG,
					 DSI_PHY_REG[i]->MIPITX_DSI_PLL_PWR,
			...
			if (0 != data_Rate) {
				...
				if (1 != dsi_params->ssc_disable) {
					...
					MIPITX_OUTREGBIT
					    (MIPITX_DSI_PLL_CON3_REG,
					     ...
					     pdelta1);
					/*DSI_OUTREGBIT(MIPITX_DSI_PLL_CON1_REG,
						DSI_PHY_REG->MIPITX_DSI_PLL_CON1,RG_DSI0_MPPLL_SDM_FRA_EN,1);*/
					DISPMSG
					    ("[dsi_drv]config:data_rate=%d, txdiv=%d, pcw=%d, delta1=%d,pdelta1=0x%x\n",
					     data_Rate, txdiv,
					     DSI_INREG32
					     (PMIPITX_DSI_PLL_CON2_REG,
					      &DSI_PHY_REG[i]->
				...
				DISPERR
				    ("[dsi_dsi.c] PLL clock should not be 0!!!\n");
				ASSERT(0);
			}
			...
			if (dsi_params->LANE_NUM > 3) {
				MIPITX_OUTREGBIT(MIPITX_DSI_DATA_LANE3_REG,
						 DSI_PHY_REG[i]->
						 MIPITX_DSI_DATA_LANE3,
						 RG_DSI_LNT3_LDOOUT_EN, 1);
			}
			/* step 16 */
			MIPITX_OUTREGBIT(MIPITX_DSI_PLL_CON0_REG,
					 DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON0,
					 RG_DSI0_MPPLL_PLL_EN, 1);

			/* step 17*/
			mdelay(1);

			MIPITX_OUTREGBIT(MIPITX_DSI_PLL_CHG_REG,
...
void DSI_PHY_TIMCONFIG(DISP_MODULE_ENUM module, cmdqRecHandle cmdq, LCM_DSI_PARAMS *dsi_params)
	{
		...
		if (0 != dsi_params->PLL_CLOCK) {
			...
			DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI", "[DISP] - kernel - DSI_PHY_TIMCONFIG, Cycle Time%d(ns)",
				cycle_time);
			DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI", "[DISP], Unit Interval%d(ns), lane#%d\n",
				ui, lane_no);
			} else {
	...
	#define NS_TO_CYCLE(n, c)	((n) / (c))
			...
			hs_trail_m = 1;
			hs_trail_n = (dsi_params->HS_TRAIL == 0) ? NS_TO_CYCLE(((hs_trail_m * 0x4) + 0x60),
				cycle_time) : dsi_params->HS_TRAIL;
			/* +3 is recommended from designer becauase of HW latency*/
			timcon0.HS_TRAIL = ((hs_trail_m > hs_trail_n) ? hs_trail_m : hs_trail_n) + 0x01;

			timcon0.HS_PRPR = (dsi_params->HS_PRPR == 0) ? NS_TO_CYCLE((0x40 + 0x5 * ui),
				cycle_time) : dsi_params->HS_PRPR;
			/* HS_PRPR can't be 1.*/
			if (timcon0.HS_PRPR < 1)
				timcon0.HS_PRPR = 1;

			timcon0.HS_ZERO = (dsi_params->HS_ZERO == 0) ? NS_TO_CYCLE((0xC8 + 0x0a * ui),
			...
			timcon1.DA_HS_EXIT	= (dsi_params->DA_HS_EXIT == 0) ?
				NS_TO_CYCLE(0xC8, cycle_time) : dsi_params->DA_HS_EXIT;

			timcon2.CLK_TRAIL	= ((dsi_params->CLK_TRAIL == 0) ?
				NS_TO_CYCLE(0x64, cycle_time) : dsi_params->CLK_TRAIL) + 0x01;
			...
			timcon3.CLK_HS_EXIT = (dsi_params->CLK_HS_EXIT == 0) ?
				NS_TO_CYCLE(0xC8, cycle_time) : dsi_params->CLK_HS_EXIT;
			timcon3.CLK_HS_POST = (dsi_params->CLK_HS_POST == 0) ? NS_TO_CYCLE((0x3c + 0x34 * ui),
 ...
 "[DISP] - kernel - DSI_PHY_TIMCONFIG, HS_TRAIL%d,HS_ZERO%d, HS_PRPR%d, LPX%d, TA_GET%d, TA_SURE%d, TA_GO%d, CLK_TRAIL%d,CLK_ZERO%d, CLK_HS_PRPR%d\n",
				...
				timcon0.HS_PRPR, timcon0.LPX, timcon1.TA_GET,
				timcon1.TA_SURE, timcon1.TA_GO, timcon2.CLK_TRAIL,
				timcon2.CLK_ZERO, timcon3.CLK_HS_PRPR);

			for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
				DSI_OUTREGBIT(cmdq, DSI_PHY_TIMCON0_REG, DSI_REG[i]->DSI_PHY_TIMECON0,
					LPX, timcon0.LPX);
				...
				DSI_OUTREGBIT(cmdq, DSI_PHY_TIMCON1_REG, DSI_REG[i]->DSI_PHY_TIMECON1,
					TA_GO, timcon1.TA_GO);
				DSI_OUTREGBIT(cmdq, DSI_PHY_TIMCON1_REG, DSI_REG[i]->DSI_PHY_TIMECON1,
				...
				DSI_OUTREGBIT(cmdq, DSI_PHY_TIMCON2_REG, DSI_REG[i]->DSI_PHY_TIMECON2,
				...
				DSI_OUTREGBIT(cmdq, DSI_PHY_TIMCON3_REG, DSI_REG[i]->DSI_PHY_TIMECON3,
				...
				DISPCHECK("%s, 0x%08x, 0x%08x, 0x%08x, 0x%08x\n", __func__,
					INREG32(&DSI_REG[i]->DSI_PHY_TIMECON0), INREG32(&DSI_REG[i]->DSI_PHY_TIMECON1),
					INREG32(&DSI_REG[i]->DSI_PHY_TIMECON2), INREG32(&DSI_REG[i]->DSI_PHY_TIMECON3));
			}

	}

void DSI_PHY_clk_switch(DISP_MODULE_ENUM module, cmdqRecHandle cmdq,
				int on)
	{
		int i = 0;
		...
		if (on) {
			DSI_PHY_clk_setting(module, cmdq,
...
DSI_STATUS DSI_EnableClk(DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
	{
			...
			for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
				 i++) {
				DSI_OUTREGBIT(cmdq, DSI_COM_CTRL_REG,
						  DSI_REG[i]->DSI_COM_CTRL, DSI_EN, 1);
			}
#endif
			return DSI_STATUS_OK;
		}
...
void DSI_Set_VM_CMD(DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
	{
		...
		if (module != DISP_MODULE_DSIDUAL) {
			for (i = DSI_MODULE_BEGIN(module);
			     i <= DSI_MODULE_END(module); i++) {
				DSI_OUTREGBIT(cmdq, DSI_VM_CMD_CON_REG,
					      DSI_REG[i]->DSI_VM_CMD_CON,
					      TS_VFP_EN, 1);
				DSI_OUTREGBIT(cmdq, DSI_VM_CMD_CON_REG,
					      DSI_REG[i]->DSI_VM_CMD_CON,
					      VM_CMD_EN, 1);
				pr_debug("DSI_Set_VM_CMD");
			}
		} else {
			DSI_OUTREGBIT(cmdq, DSI_VM_CMD_CON_REG,
...
UINT32 DSI_dcs_read_lcm_reg_v2(DISP_MODULE_ENUM module,
						cmdqRecHandle cmdq, UINT8 cmd,
						UINT8 *buffer, UINT8 buffer_size)
	{
				...
				do {
					if (max_try_count == 0)
						...
						t0.CONFG = 0x04;	/* /BTA */
						/*0xB0 is used to distinguish DCS cmd or Gerneric cmd,
							is that Right??? */
						t0.Data_ID =
							(cmd <
						...
						ret =
							wait_event_interruptible_timeout
							(_dsi_dcs_read_wait_queue[d], waitRDDone,
							 WAIT_TIMEOUT);
				if (ret > 0) {
					do {
							timeout++;
							...
							DSI_OUTREGBIT(NULL,
								...
								DSI_RACK,
								DSI_RACK, 1);
					} while (DSI_REG[d]->DSI_INTSTA.BUSY
								 && (timeout < 1000));
				}
				waitRDDone = false;
				if (timeout == 1000)
				...
				if (0 == ret) {
					...
					return 0;
						...
						/* clear interrupt */
				DSI_OUTREGBIT(cmdq, DSI_INT_ENABLE_REG,
								  DSI_REG[d]->DSI_INTSTA, RD_RDY,
								  0);
				DSI_OUTREGBIT(cmdq, DSI_INT_ENABLE_REG,
								  DSI_REG[d]->DSI_INTSTA, CMD_DONE,
								  0);
...
void DSI_set_cmdq_V2(DISP_MODULE_ENUM module, cmdqRecHandle cmdq,
			     unsigned cmd, unsigned char count,
			     ...
			     unsigned char force_update)
	{
		...
		for (d = DSI_MODULE_BEGIN(module); d <= DSI_MODULE_END(module); d++) {
			if (0 != DSI_REG[d]->DSI_MODE_CTRL.MODE) {	/* not in cmd mode */
				...
				DSI_READREG32(PDSI_VM_CMD_CON_REG, &vm_cmdq,
			...
			if (count > 1) {
				...
				vm_cmdq.CM_DATA_0 = count + 1;
				DSI_OUTREG32(cmdq, &DSI_REG[d]->DSI_VM_CMD_CON, AS_UINT32
							     (&vm_cmdq));
					...
					goto_addr =
						(unsigned long)(&DSI_CMDQ_REG[d]->data[1].byte1) + i;
							...
							DSI_MASKREG32(cmdq,
								      ...
								      &
								      (~
								       ((unsigned long)0x3u)), mask_para, set_para);
						}
		...
		if (count > 1) {
			...
			DSI_MASKREG32(cmdq, goto_addr & (~((unsigned long) 0x3u)), mask_para,
							      set_para);
		for (i = 0; i < count; i++) {
			...
			unsigned int size, unsigned char force_update)
{
	...
	do {
		...
		/*start DSI VM CMDQ*/
			...
			*(unsigned int *)(&DSI_VM_CMD_REG->data[0]), *(unsigned int *)(&DSI_VM_CMD_REG->data[1]));
				DSI_EnableVM_CMD();

			/*must wait VM CMD done?*/
			MMProfileLogEx(MTKFB_MMP_Events.DSICmd, MMProfileFlagEnd,
			*(unsigned int *)(&DSI_VM_CMD_REG->data[2]), *(unsigned int *)(&DSI_VM_CMD_REG->data[3]));
			}
		} else
#endif
		{
...
{
		memcpy((LCM_DSI_PARAMS *) dst, (LCM_DSI_PARAMS *) src,
...
int ddp_dsi_disable_irq_in_idle(unsigned int isIdle, void *cmdq_handle)
{
#if 0
	if (isIdle == 1) {
		/*DSI_OUTREGBIT(NULL, DSI_INT_ENABLE_REG,DSI_REG[0]->DSI_INTEN, FRAME_DONE_INT_EN, 0);*/
		DSI_OUTREGBIT(cmdq_handle, DSI_INT_ENABLE_REG, DSI_REG[0]->DSI_INTEN, VM_DONE, 0);
		DSI_OUTREGBIT(cmdq_handle, DSI_INT_ENABLE_REG, DSI_REG[0]->DSI_INTEN, VM_CMD_DONE, 0);
	} else {
		/*DSI_OUTREGBIT(NULL, DSI_INT_ENABLE_REG,DSI_REG[0]->DSI_INTEN, FRAME_DONE_INT_EN, 1);*/
		DSI_OUTREGBIT(cmdq_handle, DSI_INT_ENABLE_REG, DSI_REG[0]->DSI_INTEN, VM_DONE, 1);
		DSI_OUTREGBIT(cmdq_handle, DSI_INT_ENABLE_REG, DSI_REG[0]->DSI_INTEN, VM_CMD_DONE, 1);
	}
#endif
}
...
void _dump_dsi_params(LCM_DSI_PARAMS *dsi_config)
{
	...
	DISPCHECK("[DDPDSI] vact=%d, vbp=%d, vfp=%d, vact_line=%d, hact=%d, hbp=%d, hfp=%d, hblank=%d\n",
			...
			DISPCHECK("[DDPDSI] rg_bic=%d, rg_bp=%d, PLL_CLOCK=%d, dsi_clock=%d,",
			dsi_config->rg_bic, dsi_config->rg_bp, dsi_config->PLL_CLOCK, dsi_config->dsi_clock);
			DISPCHECK("ssc_range=%d, ssc_disable=%d ",
				dsi_config->ssc_range, dsi_config->ssc_disable);
			DISPCHECK("compatibility_for_nvk=%d, cont_clock=%d\n",
	...
	static void DSI_PHY_CLK_LP_PerLine_config(DISP_MODULE_ENUM module,
			cmdqRecHandle cmdq, LCM_DSI_PARAMS *dsi_params)
	{
		int i;
...
force_config:
	...
	int ddp_dsi_switch_lcm_mode(DISP_MODULE_ENUM module, void *params)
	{
		...
		if (dsi_currect_mode == mode) {
			pr_debug("[ddp_dsi_switch_mode] not need switch mode, current mode = %d, switch to %d\n",
		...
		if (lcm_cmd.cmd_if == (unsigned int)LCM_INTERFACE_DSI0)
			...
			pr_debug("dsi switch not support this cmd IF:%d\n", lcm_cmd.cmd_if);
			return -1;
		}
		...
		if (mode == 0) {
			...
			(lcm_cmd.addr << 16) | (lcm_cmd.val[0] << 24));
...
#if 0
			DSI_OUTREG32(NULL, (unsigned long)(DSI_REG[i]) + 0x130, 0x00001539 | (lcm_cmd.addr << 16) |
				(lcm_cmd.val[1] << 24));
			DSI_OUTREGBIT(NULL, DSI_START_REG, DSI_REG[i]->DSI_START, VM_CMD_START, 0);
			DSI_OUTREGBIT(NULL, DSI_START_REG, DSI_REG[i]->DSI_START, VM_CMD_START, 1);
			wait_vm_cmd_done = false;
	...
	int ddp_dsi_switch_mode(DISP_MODULE_ENUM module, void *cmdq_handle, void *params)
	{
		...
		lcm_mode_flag = lcm_cmd.cmd_if;
		if (lcm_mode_flag == (unsigned int)LCM_INTERFACE_DSI0)
			i = 0;
		else if (lcm_mode_flag == (unsigned int)LCM_INTERFACE_DSI1)
			i = 1;
		else {
			pr_debug("dsi switch not support this cmd IF:%d\n", lcm_cmd.cmd_if);
			return -1;
		...
		if (mode == 0) {
			...
			DSI_OUTREG32(cmdq_handle, (unsigned long)(DSI_REG[i]) + 0x130, 0x00001539 |
				(lcm_cmd.addr << 16)|(lcm_cmd.val[1] << 24));
			...
			DSI_OUTREG32(cmdq_handle,  (unsigned long)(DSI_REG[i]) + 0x200 + 0, 0x00001500 |
				(lcm_cmd.addr << 16) | (lcm_cmd.val[0] << 24));
			...
			DSI_OUTREG32(NULL,	(unsigned int)(DSI_REG[i])+0x200+0, 0x00001500 |
				(lcm_cmd.addr << 16) | (lcm_cmd.val[0] << 24));
...
int ddp_dsi_power_on(DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	/* if (!s_isDsiPowerOn) */
	{
		...
		if (is_ipoh_bootup) {
			...
			/* s_isDsiPowerOn = TRUE; */
			pr_debug("ipoh dsi power on return\n");
			return DSI_STATUS_OK;
		}
		...
		/* we should finish the flow, but not show on mipi interface, because the mipi interface
			is already controlled by new sleep state machine */
...
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	unsigned char read_param[4] = {0, 0, 0, 0};
	static cmdqBackupSlotHandle hSlot[4];
	...
	if (state == CMDQ_BEFORE_STREAM_SOF) {
		...
		/* send set max read packet size packet */
			t1.CONFG = 0x0;
			t1.Data_ID = 0x37;
		...
		if (dsi_i == 0) {
			ret = cmdqRecPoll(cmdq_trigger_handle, disp_addr_convert((unsigned long)(DSI_REG[i])+0xc),
					0x1, 0x00000001);
			ret = cmdqRecWrite(cmdq_trigger_handle, disp_addr_convert((unsigned long)(DSI_REG[i])+0xc),
					0x0, 0x00000001);
		}
#if 0
			else /* DSI1 */{
		...
		for (i = 0; i < 3; i++) {
			...
			for (j=0; j<4; j++) {
				if (hSlot[j]) {
					/* read from slot*/
					cmdqBackupReadSlot(hSlot[j], i, &read_data[j]);
				} else {
					/*read from dsi , support only one cmd read*/
					if (i == 0) {
						DSI_OUTREG32(NULL, &read_data[0], AS_UINT32(&DSI_REG[dsi_i]->DSI_RX_DATA0));
						DSI_OUTREG32(NULL, &read_data[1], AS_UINT32(&DSI_REG[dsi_i]->DSI_RX_DATA1));
						DSI_OUTREG32(NULL, &read_data[2], AS_UINT32(&DSI_REG[dsi_i]->DSI_RX_DATA2));
						DSI_OUTREG32(NULL, &read_data[3], AS_UINT32(&DSI_REG[dsi_i]->DSI_RX_DATA3));
					}
				}
			}

			for (j=0; j<4; j++) {
				MMProfileLogEx(ddp_mmp_get_events()->esd_rdlcm, MMProfileFlagPulse, AS_UINT32(&read_data[j]),
					AS_UINT32((unsigned int *)&((dsi_params->lcm_esd_check_table[i]).para_list[0])));

				DISPCHECK("[DSI]enter cmp read_data0 byte0 = 0x%x byte1 = 0x%x byte2 = 0x%x byte3 = 0x%x\n",
					read_data[j].byte0, read_data[j].byte1, read_data[j].byte2, read_data[j].byte3);
			}
			DISPCHECK("[DSI]enter cmp check_table cmd = 0x%x, count = 0x%x,",
				dsi_params->lcm_esd_check_table[i].cmd, dsi_params->lcm_esd_check_table[i].count);
			DISPCHECK("para_list[0] = 0x%x, para_list[1] = 0x%x, para_list[2] = 0x%x, para_list[3] = 0x%x\n",
				...
				dsi_params->lcm_esd_check_table[i].para_list[3]);
			...
			if (read_data[0].byte0 == 0x1A || read_data[0].byte0 == 0x1C) {
				read_count = read_data[0].byte2 << 8 | read_data[0].byte1;
				if (read_count > 10)   /*16-4-2*/
					read_count == 10;

				for (j=0; j<dsi_params->lcm_esd_check_table[i].count; j++) {
					if(j == 0)
						read_param[j] = read_data[1].byte0;
					if(j = 1)
						read_param[j] = read_data[1].byte1;
					if(j == 2)
						read_param[j] = read_data[1].byte2;
					if(j == 3)
						read_param[j] = read_data[1].byte3;
				}
			} else if (read_data[0].byte0 == 0x11 || read_data[0].byte0 == 0x21) {
				read_count = 1;
				read_param[0] = read_data[0].byte1;
			} else if (read_data[0].byte0 == 0x12 || read_data[0].byte0 == 0x22) {
				read_count = 2;
				read_param[0] = read_data[0].byte1;
				read_param[1] = read_data[0].byte2;
			}

			if (read_param[0] == dsi_params->lcm_esd_check_table[i].para_list[0]
				&& read_param[1] == dsi_params->lcm_esd_check_table[i].para_list[1]
				&& read_param[2] == dsi_params->lcm_esd_check_table[i].para_list[2]
				&& read_param[3] == dsi_params->lcm_esd_check_table[i].para_list[3]) {
				/* clear rx data
		...
		for (j=0; j<4; j++)
			cmdqBackupAllocateSlot(&hSlot[j], 3);
...
void PanelMaster_DSI_set_timing(UINT32 dsi_index, MIPI_TIMING timing)
{
	...
	case SSC_EN:
		...
		break;
	default:
		pr_debug("fbconfig dsi set timing :no such type!!\n");
	}
}
...
int DSI_set_roi(int x, int y)
{
	...
	pr_debug("x0=%d count=%d,read_buf[0]=%d,read_buf[1]=%d,read_buf[2]=%d,read_buf[3]=%d\n",
				x0, count, read_buf[0], read_buf[1], read_buf[2], read_buf[3]);
	if ((count == 0) || (x0 != 0)) {
		pr_debug("[DSI]x count %d read_buf[0]=%d,read_buf[1]=%d,read_buf[2]=%d,read_buf[3]=%d\n",
	...
	pr_debug("y0=%d count %d,read_buf[0]=%d,read_buf[1]=%d,read_buf[2]=%d,read_buf[3]=%d\n",
				y0, count, read_buf[0], read_buf[1], read_buf[2], read_buf[3]);
	if ((count == 0) || (y0 != 0)) {
		pr_debug("[DSI]y count %d read_buf[0]=%d,read_buf[1]=%d,read_buf[2]=%d,read_buf[3]=%d\n",
				count, read_buf[0], read_buf[1], read_buf[2], read_buf[3]);
		return -1;
	}
	return ret;
...
int DSI_test_force_te(DISP_MODULE_ENUM module)
{
	...
	if (diff > 0) {
		pr_debug("no force on TE %lld\n", diff);
		return -1;
	}

	pr_debug("force on TE %lld\n", diff);

	/*

	pr_debug("DSI_test_force_te3\n");
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
	...
	pr_debug("DSI_test_force_te4\n");

	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		DSI_OUTREGBIT(NULL, DSI_RACK_REG,DSI_REG[i]->DSI_RACK,DSI_RACK,1);
		r = wait_event_interruptible_timeout(_dsi_cmd_done_wait_queue[i],
				!(DSI_REG[i]->DSI_INTSTA.CMD_DONE), HZ/50);
		if (0 == r) {
			pr_debug("wait cmd done timeout!\n");
			ret =-1;
			break;
		}
	}*/
