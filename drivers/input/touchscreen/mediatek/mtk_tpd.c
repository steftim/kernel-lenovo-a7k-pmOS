...
#endif
#include <../drivers/cpufreq/cpufreq_poison.h>
#endif
...
#if defined(CONFIG_TOUCHSCREEN_SMARTWAKE)
	prevent_sleep = (smartwake_switch > 0) && (!in_phone_call);
	smartwake_scr_suspended = true;
#endif
#if defined(CONFIG_TOUCHSCREEN_SWEEP2WAKE)
	prevent_sleep = ((prevent_sleep) || (gestures_switch > 0) || (s2w_switch > 0)) && (!in_phone_call);
	s2w_scr_suspended = true;
#endif
#if defined(CONFIG_TOUCHSCREEN_DOUBLETAP2WAKE)
	prevent_sleep = (prevent_sleep || (dt2w_switch > 0)) && (!in_phone_call);
	dt2w_scr_suspended = true;
#endif

    if ((get_charging()) && force_enable_gestures_charging)
      charge = 1;        
    else
      charge = 0;

    if ((force_disable_gestures_HARD == true) && (!charge))
    {
        prevent_sleep = false;
    }
...
*/
...
#if defined(CONFIG_TOUCHSCREEN_SMARTWAKE)
	prevent_sleep = (prevent_sleep) || (smartwake_switch > 0);
	prevent_sleep = (prevent_sleep && !in_phone_call);
	smartwake_scr_suspended = false;
#endif
#if defined(CONFIG_TOUCHSCREEN_SWEEP2WAKE)
	prevent_sleep = (prevent_sleep) || (s2w_switch > 0);
	prevent_sleep = (prevent_sleep && !in_phone_call);
    ...
    if ((force_disable_gestures_HARD == true) && (charge == 0))
    {
        prevent_sleep = false;
    }
	if (prevent_sleep) 
