...
static int smartwake_gesture_keys[] = {
	KEY_GESTURE_SLIDE_RIGHT,
	KEY_GESTURE_SLIDE_LEFT,
	KEY_GESTURE_SLIDE_UP,
	KEY_GESTURE_SLIDE_DOWN,
	KEY_GESTURE_SLIDE_O,
	KEY_GESTURE_SLIDE_M,
	KEY_GESTURE_SLIDE_E,
	KEY_GESTURE_SLIDE_C,
...
/*
static bool gesture_is_v(int count)
{
        int i = 0;
        int sum = 0;
        bool tracking = false;
        for (i = 0; i + 1 < count; i++) {
                ...
                if ((sum == DOWN + RIGHT_DOWN + RIGHT + RIGHT_UP + UP) ||
                        (sum == RIGHT_DOWN + DOWN + RIGHT_DOWN + RIGHT + RIGHT_UP + RIGHT) ||
                        (sum == DOWN + RIGHT_DOWN + DOWN + RIGHT_DOWN + RIGHT + RIGHT_UP + UP))
...
static void check_gesture(void)
{
        ...
        if(gesture_count > 6 && key_value == 0) {
                ...
                if (gesture_is_c(gesture_count) && (support_gesture & TW_SUPPORT_C_SLIDE_WAKEUP))
                {	
                	#if SMARTWAKE_DEBUG
                		pr_info(LOGTAG"------------GESTURE _C_ DETECTED-----------");
                	#endif	
                		sprintf(wakeup_slide,"c");
                        key_value = KEY_GESTURE_SLIDE_C;
...
static void detect_smartwake(int x, int y)
{
        ...
        if (!onlyGesture) {
                if (abs(start_x - x) >= SMARTWAKE_X_DISTANCE) {
                        if (direction[0] == RIGHT && (support_gesture & TW_SUPPORT_RIGHT_SLIDE_WAKEUP)) {
                        	#if SMARTWAKE_DEBUG
                        		pr_info(LOGTAG"------------GESTURE _RIGHT_DETECTED------------");
                        #endif
                        		sprintf(wakeup_slide,"right");
                                key_value = KEY_GESTURE_SLIDE_RIGHT;
                                
                        } else if (direction[0] == LEFT && (support_gesture & TW_SUPPORT_LEFT_SLIDE_WAKEUP)) {
                        #if SMARTWAKE_DEBUG
                        		pr_info(LOGTAG"------------GESTURE _LEFT_ DETECTED------------");
                        #endif
                        		sprintf(wakeup_slide,"left");
                                key_value = KEY_GESTURE_SLIDE_LEFT;
                ...
                if (abs(start_y - y) >= SMARTWAKE_Y_DISTANCE) {
                        if (direction[0] == UP && (support_gesture & TW_SUPPORT_UP_SLIDE_WAKEUP)) {
                        	#if SMARTWAKE_DEBUG
                        		pr_info(LOGTAG"-------------GESTURE _UP_ DETECTED--------------");
                        	#endif
                        		sprintf(wakeup_slide,"up");
                                key_value = KEY_GESTURE_SLIDE_UP;
                        		

                        } else if (direction[0] == DOWN && (support_gesture & TW_SUPPORT_DOWN_SLIDE_WAKEUP)) {
                        #if SMARTWAKE_DEBUG
                        		pr_info(LOGTAG"-------------GESTURE _DOWN_ DETECTED-------------");
                        #endif
                        		sprintf(wakeup_slide,"down");
                                key_value = KEY_GESTURE_SLIDE_DOWN;
    ...
    while(gesture_p) {
        ...
        if (!strncmp(gesture, "up=",3)) {
            if(!strncmp(gesture+3, "true", 4)) {
                pr_info(LOGTAG"%s: enable up slide wakeup func.\n", __func__);
                support_gesture |= TW_SUPPORT_UP_SLIDE_WAKEUP;
            } else if(!strncmp(gesture+3, "false", 5)) {
                pr_info(LOGTAG"%s: disable up slide wakeup func.\n", __func__);
                support_gesture &= ~TW_SUPPORT_UP_SLIDE_WAKEUP;
            }
            continue;
        }
        ...
        if (!strncmp(gesture, "down=",5)) {
            if(!strncmp(gesture+5, "true", 4)) {
                pr_info(LOGTAG"%s: enable down slide wakeup func.\n", __func__);
                support_gesture |= TW_SUPPORT_DOWN_SLIDE_WAKEUP;
            } else if(!strncmp(gesture+5, "false", 5)) {
                pr_info(LOGTAG"%s: disable down slide wakeup func.\n", __func__);
                support_gesture &= ~TW_SUPPORT_DOWN_SLIDE_WAKEUP;
            }
            continue;
        }
        ...
        if (!strncmp(gesture, "left=",5)) {
            if(!strncmp(gesture+5, "true", 4)) {
                pr_info(LOGTAG"%s: enable left slide wakeup func.\n", __func__);
                support_gesture |= TW_SUPPORT_LEFT_SLIDE_WAKEUP;
            } else if(!strncmp(gesture+5, "false", 5)) {
                pr_info(LOGTAG"%s: disable left slide wakeup func.\n", __func__);
                support_gesture &= ~TW_SUPPORT_LEFT_SLIDE_WAKEUP;
            }
            continue;
        }
        ...
        if (!strncmp(gesture, "right=",6)) {
            if(!strncmp(gesture+6, "true", 4)) {
                pr_info(LOGTAG"%s: enable right slide wakeup func.\n", __func__);
                support_gesture |= TW_SUPPORT_RIGHT_SLIDE_WAKEUP;
            } else if(!strncmp(gesture+6, "false", 5)) {
                pr_info(LOGTAG"%s: disable right slide wakeup func.\n", __func__);
                support_gesture &= ~TW_SUPPORT_RIGHT_SLIDE_WAKEUP;
            }
            continue;
        }
        ...
        if (!strncmp(gesture, "c=",2)) {
            if(!strncmp(gesture+2, "true", 4)) {
                pr_info(LOGTAG"%s: enable c click wakeup func.\n", __func__);
                support_gesture |= TW_SUPPORT_C_SLIDE_WAKEUP;
            } else if(!strncmp(gesture+2, "false", 5)) {
                pr_info(LOGTAG"%s: disable c click wakeup func.\n", __func__);
                support_gesture &= ~TW_SUPPORT_C_SLIDE_WAKEUP;
            }
            continue;
        }
...
extern void touch_toggle(bool t);

void smartwake_sysfs_toggle(bool s)
{
    if (s && (smartwake_switch == 0))
    {
        smartwake_switch = 1;
        touch_toggle(1);
    }
    else if ((!s) && (smartwake_switch > 0))
    {
        smartwake_switch = 0;    
        touch_toggle(0);     
    }
