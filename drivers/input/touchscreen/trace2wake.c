...
{
    int circle_x = (x - 360) * (x - 360);
	int circle_y = circle_x + ((y - 1280) * (y - 1280));
	...
	{ 
		...
		if (!((circle_y > 67600) && (circle_y < 230400)))
		{
			trace2wake_reset();
		}
		else if ((x > 600) && (y > 1064) && (exec_count) && (checkpoint))
		{
	...
	else if (t2w_switch == 2)
	{
		...
		else if ((x > 600) && (y > 1064) && (exec_count))
		{
			trace2wake_pwrtrigger();
			exec_count = false;
		}
...
static void detect_trace2wake_right(int x, int y)
{
	...
	if (t2w_switch == 1)
	{
		...
		else if ((x < 120) && (y > 1064) && (exec_count) && (checkpoint))
		{
			trace2wake_pwrtrigger();
			exec_count = false;
		}
	...
	else if (t2w_switch == 2)
	{
		...
		else if ((x < 120) && (y > 1064) && (exec_count))
		{
			trace2wake_pwrtrigger();
			exec_count = false;
		}
...
static void t2w_input_callback(struct work_struct *unused) {
	if (((!t2w_scr_suspended) && (touch_y < 1280)) && (t2w_switch != 0))
	{
		if (init == -1)
		{
			init = 0;
		...
		if ((init_x <  150) && (init_y > 1064))
			detect_trace2wake_left(touch_x, touch_y);
		else if ((init_x >  570) && (init_y > 1064))
...
#endif
	rc = sysfs_create_file(android_touch_kobj, &dev_attr_trace2sleep.attr);
	if (rc) {
		pr_warn("%s: sysfs_create_file failed for trace2wake\n", __func__);
	}
	rc = sysfs_create_file(android_touch_kobj, &dev_attr_trace2sleep_version.attr);
	if (rc) {
