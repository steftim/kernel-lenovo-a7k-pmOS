...
bool dt2w_scr_suspended = false, exec_count = true;;
...
static int prox_init_x = 0, prox_init_y = 0 ;
...
static void set_prox_init(int x, int y)
{
...
static void detect_pocket_override_dt2w(int x, int y)
{
	if (current_tap == 0)
	{
		set_prox_init(x,y);
	...
	else //It's just a middle tap
	{
	    if(calc_within_range(prox_init_x, prox_init_y, x, y, dt2w_radius))
	        current_tap++;
	    else
	    {
	        current_tap = 0;
...
static void dt2w_input_callback(struct work_struct *unused) {
  	...
  	if (device_is_pocketed()){
       ...
       {
		detect_pocket_override_dt2w(touch_x, touch_y);
       }
  		return;
  	}
#endif
...
static int __init doubletap2wake_init(void)
{
	...
	doubletap2wake_pwrdev->phys = "dt2w_pwrkey/input2";
